    ...
    App.extend({
        handleAction: function () {
            ...
            $userInfroModal
                .modal({
                    show: false,
                    pageHeight: 480,
                    page: $.ctx + './page/system/user/_user-info.html'
                })
        ...
        handleTable: function () {
            ...
            this.table = $('.dataTable').DataTable($.po('dataTable', {
                ...
                ajax: $.ctx + 'data/system/user.json',
                rowId: 'userId',
                buttons: {
                    dom: {
                        container: {
        ...
        getAuth: function () {
            ...
            $.ajax({
                url: $.ctx + 'data/system/auth.json?userId=' + userId,
                dataType: 'JSON',
                success: function (data) {
                    if (data.success) {
                        self.authData = data;
        ...
        currentRole: function ($item) {
            var $parents = $('.page-aside-inner'),
                ...
                url = $.ctx + (typeof ID === 'undefined' ? '/data/system/user.json' : '/data/system/user.json?roleId=' + ID);
